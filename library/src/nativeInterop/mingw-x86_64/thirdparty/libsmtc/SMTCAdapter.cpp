...
static std::function<void(void*)> onPlay = [](void* data) { smtc.PlaybackRate(2.0); };
static std::function<void(void*)> onPause = [](void* data) { smtc.PlaybackRate(2.0); };
static std::function<void(void*)> onStop = [](void* data) {};
static std::function<void(void*)> onNext = [](void* data) {};
static std::function<void(void*)> onPrevoius = [](void* data) {};
...
SMTC_API int init(bool initWinRtApartment)
{
	...
	button_revoker = smtc.ButtonPressed([&](SystemMediaTransportControls smtc, SystemMediaTransportControlsButtonPressedEventArgs args) {
		...
		case SystemMediaTransportControlsButton::Play:
			onPlay(callback_data);
			break;
		case SystemMediaTransportControlsButton::Pause:
			onPause(callback_data);
			break;
		case SystemMediaTransportControlsButton::Stop:
			onStop(callback_data);
			break;
		case SystemMediaTransportControlsButton::Next:
			onNext(callback_data);
			break;
		case SystemMediaTransportControlsButton::Previous:
			onPrevoius(callback_data);
			break;
		default:
			onStop(callback_data);
			break;
	...
	repeat_revoker = smtc.AutoRepeatModeChangeRequested(
		[&](SystemMediaTransportControls sender, AutoRepeatModeChangeRequestedEventArgs args) {
			...
			case MediaPlaybackAutoRepeatMode::None:
				onLoopChanged(0, callback_data);
				break;
			case MediaPlaybackAutoRepeatMode::Track:
				onLoopChanged(1, callback_data);
				break;
			case MediaPlaybackAutoRepeatMode::List:
				onLoopChanged(2, callback_data);
				break;
			}
		}
	);
	...
	rate_revoker = smtc.PlaybackRateChangeRequested(
		[&](SystemMediaTransportControls sender, PlaybackRateChangeRequestedEventArgs args) {
			onRateChanged(args.RequestedPlaybackRate(), callback_data);
		}
	);
	...
	shuffle_revoker = smtc.ShuffleEnabledChangeRequested(
		[&](SystemMediaTransportControls sender, ShuffleEnabledChangeRequestedEventArgs args) {
			onShuffleChanged(args.RequestedShuffleEnabled(), callback_data);
		}
	);

	position_revoker = smtc.PlaybackPositionChangeRequested( 
		[](SystemMediaTransportControls smtc, PlaybackPositionChangeRequestedEventArgs args) {
			onSeek(std::chrono::duration_cast<std::chrono::milliseconds>(args.RequestedPlaybackPosition()).count(), callback_data);
		}
	);
...
SMTC_API void setOnPlay(void(*onplay)(void*))
{
	onPlay = onplay;
}

SMTC_API void setOnPause(void(*onpause)(void*))
{
	onPause = onpause;
}

SMTC_API void setOnStop(void(*onstop)(void*))
{
	onStop = onstop;
}

SMTC_API void setOnNext(void(*onnext)(void*))
{
	onNext = onnext;
}

SMTC_API void setOnPrevious(void(*onprevious)(void*))
{
	onPrevoius = onprevious;
}

SMTC_API void setOnSeek(void(*onseek)(long long, void*))
{
	onSeek = onseek;
}

SMTC_API void setOnRateChanged(void(*onrate)(double, void*))
{
...
SMTC_API void setOnLoopChanged(void(*onloop)(unsigned int, void*))
{
...
SMTC_API void setThumbnail(const wchar_t* thumbnail, bool update_when_loaded)
{
	updater = smtc.DisplayUpdater();
	loaded = false;
